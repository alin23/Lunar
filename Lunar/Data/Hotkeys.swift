...
extension AppDelegate: MediaKeyTapDelegate {
        ...
        mediaKeyTapStartingFinishTask = mainAsyncAfter(ms: 300) {
            ...
            if !self.mediaKeyTapAudioStarting {
                ...
                lazy var audioDeviceCantSetVolume = withTimeout(3.seconds, name: "CoreAudio") {
                    defaultAudioOutputDevice?.canSetVirtualMainVolume(scope: .output) == false
                } onTimeout: {
    ...
    func handle(mediaKey: MediaKey, event keyEvent: KeyEvent?, modifiers flags: NSEvent.ModifierFlags?, event: CGEvent) -> CGEvent? {
        ...
        let fine = flags.isSuperset(of: [.option, .shift])
        switch mediaKey {
        case .volumeUp:
            if Defaults[.volumeKeysControlAllMonitors] {
                ...
                increaseVolume(by: fine ? 1 : nil, for: displays, currentAudioDisplay: false)
                ...
                if (Defaults[.beepFeedback] && !flags.contains(.shift)) || (!Defaults[.beepFeedback] && flags.contains(.shift) && !fine) {
        ...
        case .volumeDown:
            if Defaults[.volumeKeysControlAllMonitors] {
                ...
                decreaseVolume(by: fine ? 1 : nil, for: displays, currentAudioDisplay: false)
