...
final class AppleNativeControl: Control {
    ...
    static func isAvailable(for display: Display) -> Bool {
        ...
        #if DEBUG
            return (display.isForTesting || display.canChangeBrightnessDS) && (display.isBuiltin || display.vendor == .apple || display.isUltraFine)
        #else
            return display.canChangeBrightnessDS && (display.isBuiltin || display.vendor == .apple || display.isUltraFine)
        #endif
        // return (
